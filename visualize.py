...
if __name__ == '__main__':
    ...
    for img_path in img_list:
        ...
        img_name = img_path.split('/')[-1][16:] ########################################

        # threshold the heatmap, use it as the segmentation map
        ret, thresh_img = cv2.threshold(output_cam, 150, 255, cv2.THRESH_BINARY) # 70 CAM 110 SEG
        ...
        elif args.mode == 'SEG_HOR_FLIP':
            cv2.imwrite('./visualize/SEG_HOR_FLIP/' + img_name, result)
            thresh_img_name = img_name.split('.')[0] + '_seg.png'
            cv2.imwrite('./visualize/SEG_HOR_FLIP/' + thresh_img_name, thresh_img)

        elif args.mode == 'SEG_VER_FLIP':
            cv2.imwrite('./visualize/SEG_VER_FLIP/' + img_name, result)
            thresh_img_name = img_name.split('.')[0] + '_seg.png'
            cv2.imwrite('./visualize/SEG_VER_FLIP/' + thresh_img_name, thresh_img)

        elif args.mode == 'SEG_ROTATION':
            cv2.imwrite('./visualize/SEG_ROTATION/' + img_name, result)
            thresh_img_name = img_name.split('.')[0] + '_seg.png'
            cv2.imwrite('./visualize/SEG_ROTATION/' + thresh_img_name, thresh_img)
